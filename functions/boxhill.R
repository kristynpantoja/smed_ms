...
BHD_m2 = function(
  ...
  ########################################################################################## treating y's indep.....
  inner.sigma.term = (diag(postpred.i$var) - diag(postpred.j$var))^2 / 
    ((error.var + diag(postpred.i$var)) * (error.var + diag(postpred.j$var)))
  inner.y.term = (postpred.i$mean - postpred.j$mean)^2 * # diff bt means
    ((error.var + diag(postpred.i$var))^(-1) + (error.var + diag(postpred.j$var))^(-1))
...
BH_m2 = function(
  ...
  for(i in 1:n){
    ...
    y.new[i] = true.function(x.new.idx[i])
