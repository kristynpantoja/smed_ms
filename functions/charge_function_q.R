...
q = function(x, mean_beta0, mean_beta1, var_mean0, var_mean1, var_e, f0, f1, type, var_margy0, var_margy1, p, alpha = NULL){
  ...
  Wass_dist = Wasserstein_distance2(mu1.temp, mu2.temp, var1.temp, var2.temp)
  q_exponent = alpha / (2 * p)
  return(1.0 / Wass_dist^q_exponent)
}
...
# charge function evaluated at x
q_2d = function(x, mean_beta0, mean_beta1, var_mean0, var_mean1, var_e, f0, f1, type, var_margy0, var_margy1, p, alpha = NULL){
  if(length(type) != 2) stop("type should be vector with length == 2")
  if(is.null(alpha)) alpha = 2 * p
  ...
  Wass_dist = Wasserstein_distance2(mu1, mu2, var1, var2)
  q_exponent = alpha / (2 * p)
  return(1.0 / Wass_dist^q_exponent)
}
...
q_gp = function(x_star, x_train, y_train, l0, l1, C_fn_type0, C_fn_type1){
  ...
  Wass_dist = Wasserstein_distance2(mu1, mu2, var1, var2, dim = 1)
