...
f_min_gpvs = function(candidate, D, Kinv0, Kinv1, subinitD, initD, y, var_e, type, l, p, alpha, buffer){
  result = q_gpvs(candidate, Kinv0, Kinv1, subinitD, initD, y, var_e, type, l, p, 
                      alpha, buffer)^k * 
    sum(apply(D, 1, function(x_i) (q_gpvs(x_i, Kinv0, Kinv1, subinitD, initD, y, var_e, type, l, p, 
                                            alpha, buffer) / sqrt(sum((x_i - candidate)^2)))^k))
  return(result)
}

# old name: add_MED_ms_oneatatime_data_gp2
add_MMEDgpvs_oneatatime = function(initD, y, type, l, subdim = NULL, var_e, N2 = 11, numCandidates = 10^5, k = 4, p = 1, 
                                         xmin = 0, xmax = 1, nugget = NULL, alpha = NULL, buffer = 0, 
                                         genCandidates = 1, candidates = NULL){
