...
# f_min_gpvsold = function(candidate, D, Kinv0, Kinv1, subdim, subinitD, initD, y, var_e, type, l, p, alpha, buffer){
#   result = q_gpvsold(candidate, Kinv0, Kinv1, subdim, subinitD, initD, y, var_e, type, l, p, 
#                       alpha, buffer)^k * 
#     sum(apply(D, 1, function(x_i) (q_gpvsold(x_i, Kinv0, Kinv1, subdim, subinitD, initD, y, var_e, type, l, p, 
...
#   D_ind = rep(NA, N2)
#   w_evals = apply(candidates, 1, FUN = function(x) Wasserstein_distance_postpred_gpvsold(x, Kinv0, Kinv1, subdim, subinitD, initD, y, var_e, type, l))
#   xoptindex = which.max(w_evals)
#   xopt = candidates[xoptindex, ]
...
#     for(i in 2:N2){
#       # Find f_opt: minimum of f_min
#       f_min_candidates = apply(candidates, 1, function(x) f_min_gpvsold(x, D[1:(i - 1), , drop = FALSE], Kinv0, Kinv1, subdim, subinitD, initD, y, var_e, type, l, p, alpha, buffer))
#       f_opt = which.min(f_min_candidates)
#       xnew = candidates[f_opt, ]
#       # Update set of design points (D) and plot new point
#       D[i, ] = xnew
