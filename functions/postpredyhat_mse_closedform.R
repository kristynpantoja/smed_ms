...
getClosedMSEyhat = function(pt, D, N, true_beta, true_type, beta_prior_mean, beta_prior_var, var_e, prior_type, indices = NULL, diagPrior = TRUE){
...
getClosedMSEyhat_seq = function(x_seq, D, N, true_beta, true_type, beta_prior_mean, beta_prior_var, var_e, prior_type, indices = NULL, diagPrior = TRUE){
  mseyhat_seq = rep(NA, length(x_seq))
  mseyhat_var_seq = rep(NA, length(x_seq))
  mseyhat_biassq_seq = rep(NA, length(x_seq))
  for(i in 1:length(x_seq)){
    mseyhat = getClosedMSEyhat(x_seq[i], D, N, true_beta, true_type, beta_prior_mean, beta_prior_var, var_e, prior_type, indices, diagPrior)
