simulate_seqMED = function(true_beta, true_type, mean_beta0, mean_beta1, var_beta0, var_beta1, var_e, 
  ...
  if(is.null(alpha_seq)){ # return space-filling design
  ...
  postvar0 = matrix(NA, length(mean_beta0), numSeq)
  postmean0 = matrix(NA, length(mean_beta0), numSeq)
  postvar1 = matrix(NA, length(mean_beta1), numSeq)
  postmean1 = matrix(NA, length(mean_beta1), numSeq)
  
  postvar0[ , 1] = diag(postvar(D, length(D), var_e, var_beta0, type[1]))
  postmean0[ , 1] = postmean(y, D, length(D), mean_beta0, var_beta0, var_e, type[1])
  postvar1[ , 1] = diag(postvar(D, length(D), var_e, var_beta1, type[2]))
  postmean1[ , 1] = postmean(y, D, length(D), mean_beta1, var_beta1, var_e, type[2])
