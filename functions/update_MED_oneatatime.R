...
f_min_data_gp = function(candidate, D, Kinv0, Kinv1, initD, y, var_e, type, p, alpha, buffer){
  result = q_data_gp(candidate, Kinv0, Kinv1, initD, y, var_e, type, p, 
                   alpha, buffer)^k * 
    sum(sapply(D, function(x_i) (q_data_gp(x_i, Kinv0, Kinv1, initD, y, var_e, type, p, 
                                         alpha, buffer) / sqrt((x_i - candidate)^2))^k))
  return(result)
}
...
add_MED_ms_oneatatime_data_gp = function(initD, y, type, l, var_e, N2 = 11, numCandidates = 10^5, k = 4, p = 2, 
  ...
  w_initD = sapply(initD, FUN = function(x) Wasserstein_distance_postpred_gp(x, Kinv0, Kinv1, initD, y, var_e, type))
  if(length(which(w_initD == 0)) != 0){
    initD = initD[-which(w_initD == 0)]
    y = y[-which(w_initD == 0)]
    w_initD = w_initD[-which(w_initD == 0)]
  ...
  if(is_xopt_in_initD){
    # Find f_opt: minimum of f_min
    f_min_candidates = sapply(candidates, function(x) f_min_data_gp(x, initD, Kinv0, Kinv1, initD, y, var_e, type, p, alpha, buffer))
    f_opt = which.min(f_min_candidates)
    xnew = candidates[f_opt]
    # Update set of design points (D) and plot new point
    D[1] = xnew
  ...
  for(i in 2:N2){
    # Find f_opt: minimum of f_min
    f_min_candidates = sapply(candidates, function(x) f_min_data_gp(x, c(initD, D[1:(i - 1)]), Kinv0, Kinv1, initD, y, var_e, type, p, alpha, buffer))
    f_opt = which.min(f_min_candidates)
    xnew = candidates[f_opt]
    # Update set of design points (D) and plot new point
    D[i] = xnew
...
add_MED_ms_oneatatime_data_gp2 = function(initD, y, type, l, var_e, N2 = 11, numCandidates = 10^5, k = 4, p = 2, 
  ...
  for(i in 2:N2){
    # Find f_opt: minimum of f_min
    f_min_candidates = sapply(candidates, function(x) f_min_data_gp(x, D[1:(i - 1)], Kinv0, Kinv1, initD, y, var_e, type, p, alpha, buffer))
    f_opt = which.min(f_min_candidates)
    xnew = candidates[f_opt]
    # Update set of design points (D) and plot new point
    D[i] = xnew
