...
f_min_data_multidim = function(candidate, D, indices0, indices1, postmean0, postmean1, postvar0, postvar1, var_e, p, k, alpha, buffer){
  result = q_data_multidim(candidate, indices0, indices1, postmean0, postmean1, postvar0, postvar1, var_e, p, 
                   alpha, buffer)^k * 
    sum(apply(D, 1, function(x_i) (q_data_multidim(x_i, indices0, indices1, postmean0, postmean1, postvar0, postvar1, var_e, p, 
                                         alpha, buffer) / sqrt(sum((x_i - candidate)^2)))^k))
  return(result)
}

# multidimensional case
add_MED_ms_oneatatime_data_multidim = function(initD, inity, mean_beta_full, beta_true = NULL, indices_true = NULL, 
  ...
  if(wasserstein0 == 1){
    w_initD = apply(initD, 1, FUN = function(x) Wasserstein_distance_postpred_multidim(x, indices0, indices1, postmean0, postmean1, postvar0, postvar1, var_e))
    if(length(which(w_initD == 0)) != 0){
      initD = initD[-which(w_initD == 0), ]
  ...
  w_cand = apply(candidates, 1, FUN = function(x) Wasserstein_distance_postpred_multidim(x, indices0, indices1, postmean0, postmean1, postvar0, postvar1, var_e))
  ...
  if(is_x_max_in_initD){
    ...
    # N2 = N2 - 1
    # Find f_opt: minimum of f_min
    f_min_candidates = apply(candidates, 1, function(x) f_min_data_multidim(x, initD, indices0, indices1, postmean0, postmean1, postvar0, postvar1, var_e, p, k, alpha, buffer)) # why was this running before changing sapply to apply?
    f_opt = which.min(f_min_candidates)
    xnew = candidates[f_opt, ]
    # Update set of design points (D) and plot new point
    D[1, ] = xnew
  ...
  for(i in 2:N2){
    # Find f_opt: minimum of f_min
    f_min_candidates = apply(candidates, 1, function(x) f_min_data_multidim(x, rbind(initD, D[1:(i - 1), , drop = FALSE]), indices0, indices1, postmean0, postmean1, postvar0, postvar1, var_e, p, k, alpha, buffer))
    f_opt = which.min(f_min_candidates)
    xnew = candidates[f_opt, ]
    # Update set of design points (D) and plot new point
    D[i, ] = xnew
