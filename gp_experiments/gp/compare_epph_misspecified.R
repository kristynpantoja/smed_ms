for(scenario in c(3, 4, 5, 6)){
  ...
  for(j in 1:numSims){
    ...
    PPH_seq.qc$Design = "qcap"
    PPH_seq.lo$Design = "SeqMED" # "leaveout"
    PPH_seq.qc2$Design = "keepq2"
  ...
  # PPHmean_seq = rbind(dplyr::filter(PPH0mean_seq, Design == "bh"),
  #                     dplyr::filter(PPH1mean_seq, Design == "bh"),
  #                     dplyr::filter(PPHTmean_seq, Design == "bh"))
