for(scenario in c(1, 2)){
  ...
  # purpose: to illustrate seqmed in scenarios 1 & 2
  ...
  Nin = 3
  numSeq = 1 ### illustrate 1 stage
  seqN = 6 ### with 6 points in it
  Nnew = numSeq * seqN
  Nttl = Nin + Nnew
  xmin = 0
  xmax = 1
  ...
  ################################################################################
  # initial design
  x_input_idx = sample(1:numx, Nin, replace = FALSE)
  ...
  H0_predfn = getGPPredictive(
    x_seq, x_train, y_train, model0$type, model0$l, model0$p, model0$signal.var, 
    model0$measurement.var)
  H1_predfn = getGPPredictive(
    x_seq, x_train, y_train, model1$type, model1$l, model1$p, model1$signal.var, 
    model1$measurement.var)
  
  # get w_seq
  Kinv0 = solve(getCov(
    x_train, x_train, model0$type, model0$l, model0$p, model0$signal.var))
  Kinv1 = solve(getCov(
    x_train, x_train, model1$type, model1$l, model1$p, model1$signal.var))
  w_seq = sapply(x_seq, FUN = function(x1) 
    WNgp(x1, Kinv0, Kinv1, x_train, y_train, model0, model1))
  ...
  plt = ggplot(data = ggdata.melted, aes(x = x, y =value, color = variable), 
         linetype = 1) + 
    geom_path() + 
