...
seqmed.m = SeqMEDgp(
  ...
  candidates = x_seq, function.values = y_seq, 
  model0 = model0, model1 = model1, 
  numSeq = numSeq, seqN = seqN, prints = TRUE, buffer = 0, 
  objective.type = 1, noise = TRUE, measurement.var = sigmasq_measuremt)

all.equal(seqmed.m$x.new, seqmed.b$x.new) # why tho?

# browser for seqmed.b -- is buffer staying nonzero? not being overwritten?
...
seqmed.b = SeqMEDgp(
  ...
  numSeq = numSeq, seqN = seqN, prints = TRUE, buffer = buffer, 
  objective.type = 1, noise = TRUE, measurement.var = sigmasq_measuremt)
# Browse[2]> WNgp(x, Kinv0, Kinv1, initD, y, model0, model1)
# [1] 9.96668e-06
# Browse[2]> buffer
# [1] 1e-15
# Browse[2]> WNgp(x, Kinv0, Kinv1, initD, y, model0, model1) + buffer
# [1] 9.96668e-06
# Browse[2]> Q

## see q_augdist.R for more
