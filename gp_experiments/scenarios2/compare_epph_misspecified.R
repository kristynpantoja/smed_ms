for(scenario in c(3.2, 4.2, 5.2, 6.2)){
  for(input.type in 1:3){
    ...
    for(j in 1:numSims){
      ...
      PPH_seq.sm = getPPHseq(sm, model0, model1, modelT)
      ...
      PPH_seq.tmp = rbind(
        PPH_seq.bh, PPH_seq.q, PPH_seq.q1, PPH_seq.sm, PPH_seq.b, PPH_seq.r, PPH_seq.sf)
      PPH_seq.tmp$sim = j
    ...
    ggsave(
      filename = paste0("20210604_scen", scenario, "_in", input.type, "_epph.pdf"), 
      plot = epph.plt, 
