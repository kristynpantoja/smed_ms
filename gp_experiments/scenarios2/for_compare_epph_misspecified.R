for(scenario in c(3.2, 4.2, 5.2, 6.2)){
  for(input.type in 1:3){
    ...
    # scenario = 3.2 # scenarios: 3.2, 4.2, 5.2, 6.2
    ...
    for(j in 1:numSims){
      ...
      PPH_seq.m = getPPHseq(m, model0, model1, modelT)
      # master data frame
      ...
      PPH_seq.m$type = "signal"
      PPH_seq.tmp = rbind(
        PPH_seq.bh, PPH_seq.q, PPH_seq.b, PPH_seq.r, PPH_seq.sf, PPH_seq.m)
      PPH_seq.tmp$sim = j
