...
source(paste(functions_home, "/gp_plot.R", sep = ""))
...
HT_predfn = getGPPredictive(x_seq, x_input, y_input, typeT, lT, 1, measurement.var = sigmasq_measuremt)
...
H0_predfn = getGPPredictive(x_seq, x_input, y_input, type01[1], l01[1], 1, measurement.var = sigmasq_measuremt)
...
H1_predfn = getGPPredictive(x_seq, x_input, y_input, type01[2], l01[2], 1, measurement.var = sigmasq_measuremt)
...
ggplot(ggdata) + 
  ...
  geom_path(data = ggdata0, aes(x = x, y = `HT Pred Mean`), color = gg_color_hue(5)[4]) +
  geom_ribbon(data = ggdata1, 
              aes(x = x, ymin = lower, ymax = upper), 
              color = gg_color_hue(5)[5], linetype = 2, 
              fill = gg_color_hue(5)[5], alpha = 0.1) +
  geom_path(data = ggdata1, aes(x = x, y = `HT Pred Mean`), color = gg_color_hue(5)[5]) +
