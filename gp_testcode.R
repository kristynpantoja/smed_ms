...
# Get covariance matrix from Covariance function
covariance_xstar = C_fn(xstar, xstar)
...
dat <- data.frame(x=xstar, t(values_noise))
dat <- melt(dat, id="x")

dat3 = data.frame(xstar, E_f_noise)

fig3 <- ggplot(dat,aes(x=x,y=value)) +
  geom_ribbon(data=dat3, 
              aes(x=xstar, y=E_f_noise, ymin=(E_f_noise-2*sqrt(diag(C_f_noise))), 
                  ymax=(E_f_noise+2*sqrt(diag(C_f_noise)))),
              fill="grey80") + # Var
  geom_line(aes(color=variable)) + #REPLICATES
  geom_line(data=dat3,aes(x=xstar,y=E_f_noise), size=1) + #MEAN
  geom_point(data=obs,aes(x=x,y=y)) +  #OBSERVED DATA
  scale_y_continuous(lim=c(-3,3), name="output, f(x)") +
  xlab("input, x")
fig3
