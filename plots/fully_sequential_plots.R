...
#     #   candidates, 
#     #   function(x) f_min_seqmed(x, seqmed1$D[1:prev.idx], postmean0, postmean1, 
#     #                            postvar0, postvar1, var_e, type, p, k, alpha, buffer))
...
#       ymax = apply(cbind(f1est_seq, f2est_seq), 1, max)
#     )
#     plt0.tmp = ggplot(data.frame(x = seqmed1$D[1:i])) +
#       geom_histogram(binwidth = 0.12, closed = "right", aes(x =x, y = after_stat(density))) +
#       theme_bw() +
#       theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())
#     plt1.tmp = ggplot(ggdata, aes(x = x, y = y, color = Function, linetype = Function)) +
#       scale_linetype_manual(values = c(2, 2, 1, 1)) +
#       scale_color_manual(values = c(gg_color_hue(3)[c(1, 3)], "black", gg_color_hue(3)[2])) +
#       geom_path() +
#       geom_ribbon(data = ggdata_ribbon, mapping = aes(x = x, ymin = ymin, ymax = ymax), alpha = 0.2,
...
#       theme_bw() +
#       theme(panel.grid.minor = element_blank())
#     ggarrange(plt0.tmp, plt1.tmp, widths = c(1, 2))
#   }
# }
...
# import the simulations
seqmed_file0 = paste0(output_home, "/seqmed/scenario2_seqmed_simulations", 
                      "_numSeq", numSeq, 
                      "_seqN", seqN,
                      "_numSims", numSims)
bh_file0 = paste0(output_home, "/boxhill/scenario2_boxhill_simulations", 
                  "_N", N, 
                  "_MMEDinput", as.numeric(MMEDinputdata),
                  "_numSims", numSims)
if(isParallelized){
