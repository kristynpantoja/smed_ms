...
for(i in 1:numSims){
  ...
  f1dsims[[i]] = generate_SMMEDgpvs(y_seq, initD = x_input, initD_indices = x_input_ind, 
...
for(i in 1:numSims){
  ...
  f2dsims[[i]] = generate_SMMEDgpvs(y_seq, initD = x_input, initD_indices = x_input_ind, 
                                    inity = y_input, type_hypotheses = type01, l_hypotheses = l01, 
                                    indices0 = indices0, indices1 = indices1, var_e = 1, N2 = N2, 
                                    k = k, p = p, xmin = xmin, xmax = xmax, nugget = nugget, 
                                    numSeq = N2, N_seq = 1, alpha_seq = alpha, buffer_seq = 0, 
...
saveRDS(sims, paste(output_home, "gpvs_smmedsims.rds", sep = ""))
