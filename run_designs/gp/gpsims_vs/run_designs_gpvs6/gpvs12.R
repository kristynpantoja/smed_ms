...
output_home = paste(home,"/run_designs_gpvs6/",sep="")
...
for(i in 1:numSims){
  ...
  mmedf1dsims[[i]] = add_MMEDgpvs(x_input, y_input, type01, l01, indices0, indices1,
                                  var_e = 1, N2, numCandidates = NULL, k, p, xmin, 
                                  xmax, nugget, alpha, buffer = 0, 
                                  candidates = x_grid, algorithm = 1)
  
  smmedf1dsims[[i]] = generate_SMMEDgpvs(y_seq, initD = x_input, initD_indices = x_input_ind, 
                     inity = y_input, type_hypotheses = type01, l_hypotheses = l01, 
                     indices0 = indices0, indices1 = indices1, var_e = 1, N2 = N2, 
                     k = k, p = p, xmin = xmin, xmax = xmax, nugget = nugget, 
                     numSeq = N2, N_seq = 1, alpha_seq = alpha, buffer_seq = 0, 
                     candidates = x_grid, seed = seed_i)
}
...
for(i in 1:numSims){
  ...
  mmedf2dsims[[i]] = add_MMEDgpvs(x_input, y_input, type01, l01, indices0, indices1,
                                  var_e = 1, N2, numCandidates = NULL, k, p, xmin, 
                                  xmax, nugget, alpha, buffer = 0, 
                                  candidates = x_grid, algorithm = 1)
  
  smmedf2dsims[[i]] = generate_SMMEDgpvs(y_seq, initD = x_input, initD_indices = x_input_ind, 
                                    inity = y_input, type_hypotheses = type01, l_hypotheses = l01, 
                                    indices0 = indices0, indices1 = indices1, var_e = 1, N2 = N2, 
                                    k = k, p = p, xmin = xmin, xmax = xmax, nugget = nugget, 
                                    numSeq = N2, N_seq = 1, alpha_seq = alpha, buffer_seq = 0, 
...
sims = list("x_input_ind" = x_input_ind_mat, 
            ...
            "mmedgp_f1dsims" = mmedf1dsims, 
            "mmedgp_f2dsims" = mmedf2dsims,
            "smmedgp_f1dsims" = smmedf1dsims, 
            "smmedgp_f2dsims" = smmedf2dsims)

saveRDS(sims, paste(output_home, "gpvs_sims.rds", sep = ""))
