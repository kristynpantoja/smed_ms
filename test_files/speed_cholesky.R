...
for(i in 1:numSims){
  ...
  if(i == numSims){
    ...
    plot(x_seq, y_seq, type = "l", ylim = range(y_seq, postpredmu0, postpredmu1), ylab = "sample function",
         main = "OLD WAY") # plot the function
    ...
    legend("topleft", legend = c("train y", "true y", "H0:gaussian", "H1:periodic"),
...
# NEW #
#######
# tried to take away solves and use cholesky+forwardsolve, but it wasn't actually faster.
# turns out many multiplications and two solves is better than many forwardsolves
...
mmed_gp_list2 = list()
# mmed points (x and y)
newpts_mat2 = matrix(NA, N2, numSims)
newpts_ind_mat2 = matrix(NA, N2, numSims)
truey_mat2 = matrix(NA, N2, numSims)
...
for(i in 1:numSims){
  ...
  # generate mmed
  mmed_gp_list2[[i]] = add_MED_ms_oneatatime_data_gp_NEW(x_train, y_train, type01, l01, var_e = 1, N2 = N2, 
                                                    k = k, p = p, xmin = xmin, xmax = xmax, 
                                                    nugget = nugget, alpha = alpha, buffer = 0, candidates = x_seq)
  
  # mmed inputs' predictions and evaluations
  newpts = mmed_gp_list[[i]]$addD
  newpts_mat2[ , i] = newpts
  newpts_ind = mmed_gp_list[[i]]$indices
  newpts_ind_mat2[ , i] = newpts_ind
  truey = y_seq[newpts_ind]
  truey_mat2[ , i] = truey
  if(i == numSims){
    H0_pred = getGPPredictive(newpts, x_train, y_train, type01[1], l01[1], nugget = NULL)
    ...
    postpredmu1 = H1_pred$pred_mean
    plot(x_seq, y_seq, type = "l", ylim = range(y_seq, postpredmu0, postpredmu1), ylab = "sample function",
         main = "NEW WAY") # plot the function
    points(x = x_train, y = y_train)
    H0_predfn = getGPPredictive(x_seq, x_train, y_train, type01[1], l01[1], nugget = NULL)
    H1_predfn = getGPPredictive(x_seq, x_train, y_train, type01[2], l01[2], nugget = NULL)
    lines(x = x_seq, y = H0_predfn$pred_mean)
...
NEWtrain1sims = list("grid" = x_seq,
                     "sim_fns" = y_seq_mat,
                     "x_train" = x_train,
                     "x_train_ind" = x_train_ind,
                     "x_spacefill" = x_spacefill,
                     "x_spacefill_ind" = x_spacefill_ind,
                     "mmed_gp_list" = mmed_gp_list2,
                     "x_mmed" = newpts_mat2,
                     "x_mmed_ind" = newpts_ind_mat2,
                     "y_mmed" = truey_mat2)
